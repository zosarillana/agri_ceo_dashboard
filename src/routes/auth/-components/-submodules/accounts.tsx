...
// Status badge config — mirrors STATUS_OPTIONS from the input form
...
> = {
  ...
  unpaid: {
    label: "Unpaid",
    className: "text-rose-600   border-rose-500/40   bg-rose-500/10",
  },
  pending: {
    label: "Pending",
    className: "text-amber-600  border-amber-500/40  bg-amber-500/10",
      ...
      <TableBody>
        {accounts.map((a) => (
          <TableRow
            key={a.id}
            className={
              a.status === "paid" || a.status === "received" ? "opacity-50" : ""
            }
          >
...
export default function AccountsDash() {
  ...
  const payables = accounts.filter(
