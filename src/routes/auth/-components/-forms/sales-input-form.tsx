...
interface SaleRow {
  id?: number; // Added id for existing records
  product_id: number;
  market: Market;
  sales: number;
  quantityKg: number;
  ...
  isReadOnly: boolean;
  originalSale?: Sale; // Store original sale data for reference
}
  ...
  return Object.fromEntries(
    products.map((p) => {
      ...
      return [
        p.id,
        {
          id: existing?.id, // Store the id for updates
          product_id: p.id,
          ...
          isReadOnly: !!hasData,
          originalSale: existing, // Store original for reference
        },
      ];
    }),
  );
...
export default function SalesInputForm({
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    // Build payload with ids for updates
    const payload = rowsToSave.map((r) => {
      ...
      // Include id if this is an existing record being updated
      if (r.id) {
        basePayload.id = r.id;
      }
    ...
    try {
      ...
      setStatus("success");
      setStatusMsg(
        `Sales ${rowsToSave.some((r) => r.id) ? "updated" : "saved"} for ${format(isoToDate(dateISO), "PPP")}.`,
      );
      onSaved();
    } catch (err: any) {
      setStatus("error");
